...
public class LinearRetryTests
{
    ...
    {
        var retry = new LinearRetry(default, 3);
    ...
    public void ShouldRetry_ShouldReturn_LinearRetryInterval(uint currentRetryCount, int expectedRetryInterval)
    {
        ...
        _ = retry.ShouldRetry(currentRetryCount, HttpStatusCode.RequestTimeout, out var retryInterval);
    ...
    {
        _ = Assert.Throws<ArgumentOutOfRangeException>(() => new LinearRetry(TimeSpan.FromSeconds(-4), 3));
    }
}
